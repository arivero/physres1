...
\section{From Newton to the Path
Integral}\label{from-newton-to-the-path-integral}
...
\subsection{2. Notation and Claim
Taxonomy}\label{notation-and-claim-taxonomy}

\subsubsection{2.1 Core Objects}\label{core-objects}
...
\subsubsection{2.2 Weak-Form Preliminaries for Point-Like
...
\subsubsection{2.3 Claim Taxonomy}\label{claim-taxonomy}
...
\subsubsection{2.4 Seed Claims for the
...
\subsubsection{2.5 Scope Boundary
...
\subsection{3. Newtonian Refinement and Area
Law}\label{newtonian-refinement-and-area-law}

\subsubsection{3.1 Source-Critical
Framing}\label{source-critical-framing}
...
\subsubsection{3.2 Discrete Refinement
...
\subsubsection{3.3 Continuum Passage and Central-Force
...
\subsubsection{3.4 Closed Question from the Section 2
...
\subsection{4. Action as Additive
Invariant}\label{action-as-additive-invariant}

\subsubsection{4.1 Stationarity Setup}\label{stationarity-setup}
...
\subsubsection{4.2 Euler-Lagrange
...
\subsubsection{4.3 Rotational Symmetry and Angular
...
\subsubsection{4.4 Additivity and Composition
...
probes of extrema can be expressed in distributional language; the idea
appears in explicit form in {[}Rivero9803035{]}. In this manuscript,
technical use of such probes is deferred to Section 5, where mollifier
limits and admissibility are stated explicitly.
...
\subsection{5. Dirac Distributions and Extremal
Action}\label{dirac-distributions-and-extremal-action}

\subsubsection{5.1 Why Weak Formulations Appear
...
\subsubsection{5.2 Weak Euler-Lagrange
...
\subsubsection{5.3 Point-Like Probes via Mollifiers (Not Raw
...
\subsubsection{5.4 Corners and Impulses: Jump
...
\subsubsection{5.5 Extremal Measures: Finite-Dimensional Analogy and
...
``formal measures'' supported on stationary-action trajectories. A
concrete version of this idea, and its relation to exponential weighting
and consistency, is developed in {[}Rivero9803035{]}. In this manuscript
we treat such expressions as roadmap heuristics until they are
regularized and made compatible with composition (Section 6).

\subsubsection{5.6 Caveats (Nonlinear Distribution
...
\subsection{6. Composition and Path
Integral}\label{composition-and-path-integral}

\subsubsection{6.1 Composition Postulate for
...
\] interpreting the Gaussian integral in Euclidean time or with the
usual \(i0\) prescription. Then \[
...
needed for refinement-stable kernel composition.

\subsubsection{6.2 From Additive Action to Multiplicative
...
\subsubsection{6.3 Ordering, Discretization, and Quantum
...
\subsubsection{6.4 Formal Continuum Limit and Stationary
...
in \texttt{Heuristic\ H4.0}.

\subsubsection{6.5 Link Back to Section 5 Singular
...
\subsection{7. Deformation Quantization
Bridge}\label{deformation-quantization-bridge}

\subsubsection{7.1 From Path Weights to Product
...
\subsubsection{7.2 Commutator-to-Poisson
...
\subsubsection{7.3 Concrete Model and Ordering
...
corrections {[}Rivero0302285{]}.

\subsubsection{7.4 Equivalence Classes and Groupoid
...
\subsubsection{7.5 Formal Deformation
...
\subsection{8. Renormalization as Controlled
Refinement}\label{renormalization-as-controlled-refinement}

\subsubsection{8.1 Why Renormalization Appears in Refinement
...
\subsubsection{8.2 Regulator, Bare Data, and Renormalized
...
\subsubsection{8.3 RG Equation from Cutoff
...
\subsubsection{8.4 Refinement Control Before QFT: Scale-Halving as a
...
corresponding composition law forms a group (the Butcher group). Rivero
observes that ``scale-halving then rescaling back'' is an RG-like
operation on method coefficients, and the same rooted-tree/Hopf-algebra
combinatorics appears in perturbative renormalization bookkeeping
{[}RiveroOde2002{]} {[}Brouder1999{]} {[}McLachlan2017{]}
{[}ConnesKreimer2000{]}.
...
\subsubsection{8.5 Counterterms as Refinement
...
where the ``\(\cdots\)'' stands for additional operators allowed by
symmetries and by the desired accuracy. The counterterm viewpoint is
simply the statement that \(Z,m^2,\lambda,\ldots\) must be chosen as
functions of \(\Lambda\) so that the cutoff-stable limits of observables
exist. In this compositional narrative, counterterms are refinement
corrections required to keep the ``same theory'' after integrating out
short scales.
...
remainder. (This ``derivative as renormalized limit'' viewpoint is
stated explicitly in {[}RiveroOde2002{]}.)
...
\subsubsection{8.6 Assumptions and Boundaries
...
\subsection{9. Unified Perspective and Open
Problems}\label{unified-perspective-and-open-problems}

\subsubsection{9.1 End-to-End Claim Graph}\label{end-to-end-claim-graph}
...
\subsubsection{9.2 Transition Stress Test}\label{transition-stress-test}
...
\subsubsection{9.3 What Is Proven vs
...
\def\labelenumi{\arabic{enumi}.}
\item
  Section 3: polygonal central-impulse refinement preserves angular
  momentum and equal areas at finite step. Status: \texttt{Proposition}
  + \texttt{Derivation}. Main boundary: central impulses and consistent
  refinement limit.
\item
  Section 4: Euler-Lagrange plus Noether recover angular-momentum and
  energy invariants. Status: \texttt{Derivation} + \texttt{Proposition}.
  Main boundary: \(C^2\) trajectory regularity and standard variational
  hypotheses.
\item
  Section 5: weak Euler-Lagrange, mollifier probes, and jump laws for
  corners/impulses. Status: \texttt{Proposition} + \texttt{Derivation}.
  Main boundary: distribution linearity and no undefined nonlinear
  products.
\item
  Section 6: composition plus additivity imply exponential weighting;
  stationary phase yields classical recovery. Status: mixed
  (\texttt{Proposition} + \texttt{Derivation} + \texttt{Heuristic}).
  Main boundary: formal path-integral usage and local stationary-phase
  assumptions.
\item
  Section 7: deformation products recover the Poisson bracket; ordering
  appears as deformation-equivalence choice. Status: mixed
  (\texttt{Proposition} + \texttt{Derivation} + \texttt{Heuristic}).
  Main boundary: formal/asymptotic deformation setting and scope of
  equivalence.
\item
  Section 8: RG appears as semigroup consistency under composed
  refinement; counterterms appear as refinement corrections. Status:
  mixed (\texttt{Proposition} + \texttt{Derivation} +
  \texttt{Heuristic}). Main boundary: closure/truncation assumptions and
  regulator-scheme choice.
\end{enumerate}
...
\subsubsection{9.5 Next-Paper Agenda}\label{next-paper-agenda}

The remaining extensions beyond the continuation appendices are: 1. Add
a fully model-specific loop computation (beyond the template-level
calculation in section \texttt{10.1}). 2. Add a short dimensional-aside
appendix (from \texttt{{[}RiveroSimple{]}}) clearly labeled as
heuristic/exploratory. 3. Add one diagram-level summary figure of the
compatibility chain for publication readability. 4. Add a final
notation-normalization pass for submission formatting.

Roadmap status: with Section 9, the planned section sequence through
Section 9 is complete.

\subsection{10. Technical Appendices (Continuation
Stages)}\label{technical-appendices-continuation-stages}

This section executes the continuation appendices announced at the end
of Section 9. Each subsection is a compact worked extension tied to one
residual vulnerability.

\subsubsection{10.1 Worked Renormalization Template (Single
...
\subsubsection{10.2 Ordering/Discretization Pair with Same Classical
...
Section 7 transition claim {[}Rivero0302285{]} {[}Landsman1998{]}.

\subsubsection{10.3 Foundational Compatibility
Principle}\label{foundational-compatibility-principle}

This appendix replaces the earlier continuation draft with a
foundational concept aimed at the manuscript's core objective.
...
\texttt{Derivation\ D10.1\ (Bridge\ to\ sections\ 3-\/-8).} 1. Section
3/Section 4 provide partition compatibility via area-law and
Noether-action equivalence. 2. Section 6/Section 7 provide
representation compatibility via ordering/deformation equivalence with
identical \(\hbar\to0\) limit. 3. Section 8 provides scale compatibility
via RG semigroup consistency.
...
\subsubsection{10.4 Continuation Outcome}\label{continuation-outcome}

Continuation appendices in sections 10.1--10.3 close the three technical
gaps identified in Section 9: 1. explicit renormalization subtraction
and running, 2. explicit ordering/discretization \(O(\hbar)\) shift with
fixed classical limit, 3. explicit foundational compatibility principle
unifying the full chain.

These additions do not alter the thesis; they increase computational
accountability of the existing chain. Operational note: four
bibliography items are intentionally deferred as \texttt{PENDING} for
later local-PDF ingestion (\texttt{Dirac1933}, \texttt{Feynman1948},
\texttt{Connes1994}, \texttt{Landsman1998}).

\subsubsection{10.5 Singular Contact Interaction as an Explicit RG
Computation (2D
