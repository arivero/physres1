...
For the explicit Euler map \(E_h(y):=y+h f(y)\), expanding one order
further gives \[
E_{h/2}\!\circ E_{h/2}(y)-E_h(y)
=\frac{h^2}{4}\,f'(y)[f(y)]+\frac{h^3}{16}\,f''(y)[f(y),f(y)]+O(h^4),
\] where \(f''(y)[v,w]\) is the bilinear second derivative. In
rooted-tree language, the new \(O(h^3)\) term is the order-3 branch-tree
elementary differential \(F([\bullet,\bullet])\).
