...
- **Idle-agent monitoring (mandatory):** Watch for agents that go idle without reporting `done:` or `self:`. If an agent is idle mid-task, message to diagnose. If an agent has finished all tasks and the kanban is empty for them, **proactively message them** to either: (a) propose a new task (`kanban.sh propose <src> "<task>"`), or (b) take a Philosophenweg walk (`kanban.sh self <agent> <src> "Philosophenweg (<name>)"` — see `agents/shared/philosophenweg.md`). **Agents do not rest.** There is always something to propose, explore, or walk on. "Rest" is not an option — if an agent is out of ideas, send them to the Philosophenweg.
- **CRITICAL — After any agent reports findings or results, the orchestrator must check that every actionable follow-up has a kanban row. If the reporting agent did not seed the rows (findings, 2nd-agent review requests, patch drafts), the orchestrator seeds them using `kanban.sh propose <source> "<task>"`.** Never edit `meta/kanban.md` directly — always use the script so the assignee field is empty and agents can self-service via `kanban.sh claim`. No finding or ONE-AGENT result may be left only in a status file or blackboard — it must enter the kanban before the session ends.
- **CRITICAL — Kanban is the sole task facility. Messages may SUGGEST, never ASSIGN.** The orchestrator must NEVER assign tasks to agents via `SendMessage`. The kanban skill (`skills/kanban/`) is the only mechanism for task assignment. To pre-assign a task: `kanban.sh self <agent> or "<task>"`. To leave unclaimed: `kanban.sh propose or "<task>"`. Agents self-service from the kanban — they read it, claim tasks, and report done entirely on their own. A message saying "please do X" has no binding force; agents are not required to comply unless X is in the kanban. The orchestrator's role in task flow is to seed and signal the kanban, not to direct agents by conversation. To redirect agents away from current work, use `kanban.sh signal "DO SUGGESTIONS"` (pauses new starts), update the kanban, then restore `kanban.sh signal "GOOD DAY"`. **Note:** Claude Code's built-in TaskCreate/TaskList/TaskUpdate tools are NOT used in this project — the kanban skill is the sole task facility.
