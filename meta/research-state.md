...
- **D12 convention mismatch (sev-2, R19 PRIORITY 1)**: D12.1 uses H=-Δ (kernel 4π), but D12.3 Mehler free-limit and P12.2 both use H=-(1/2)Δ (kernel 2π). D12.3b explicit formula wrong. Full fix scope: (1) D12.1: 4π→2π, 4(t+ε)→2(t+ε); (2) P12.1 dominator: K_0(t/2) → (2πt)^{-d/2}exp(-|x-y|²/(3t)); (3) D12.3b explicit formula: 4πt→2πt, |x-y|²/(4t)→|x-y|²/(2t). Conclusion of all props correct. Critic SINGLE-AGENT (proposals/critic-edit-d12-convention-fix.md) — needs 2nd agent + D12.3b addition to proposal.
