...
- **SPECTRAL INVISIBILITY (R26, BB3)**: Δ_{1/2}=|g|^{1/4}(-Δ)|g|^{-1/4} is isospectral to -Δ_g (conjugation/similarity). Gilkey endomorphism E'=0 universally (drift cancels V_HD). Therefore a_k(Δ_{1/2})=a_k(-Δ_g), a_1=R/6. THREE-AGENT (critic+mathematician+physicist). "HD" coefficients 2^k/k! are for O3=-Δ+V_HD (different operator). Retraction: `patches/critic-patch-retract-D91pq.md` (correct, TWO-AGENT).
- **BB4 (R26)**: SD coefficients for O3/bare/conformal on S³. a_k(O3)=2^k/k!, a_k(bare=Δ_{1/2})=1/k!, a_k(conf)=0. CMP9b.4: a₂(conf;S^d)=-d(d-1)(d-3)/180 (D=4 coincidence for conformal). CMP11.1 (a₃), CMP12.1 (a₄) confirmed. PH10.1 perfect-square. BB4 at 439 lines, needs promotion.
- **BB5 promoted** → `notebooks/vhd-product-warped-additivity.md` (FOUR-AGENT: C5.1 + warped product + coord-dep; critic).
- **BB1 (R26)**: Overwritten with spectral invisibility analysis. Physical content of HD normalization: measure choice (composition), not spectral modification. D=4 coincidences survive (concern V_HD function, not a_k).
- **OQ-FdV-3 RESOLVED** (THREE-AGENT R24): HC proof: V_HD=−|ρ|² interpretation. UNAFFECTED by spectral invisibility (concerns V_HD geometry).
