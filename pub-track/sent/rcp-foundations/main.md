...
composition law for the scale channel.
...
**Theorem (from [Main] Proposition P4.2; assumed here as hypothesis — see proof sketch below).** Consider action-based dynamics on \(\mathbb{R}^d\) with mass parameter \(m\) and self-adjoint generator, and let \(\{K(x,y;t)\}_{t>0}\) be a family of transition kernels satisfying:
