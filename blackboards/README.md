...
| 5 | Rank-2 instanton lattice + mixed Stokes constants from (C). Rank-2 trivial (corollary D6.4b). NEW: mixed-sector factorization C_{n,m} = C_{1,0}^n · C_{0,1}^m forced by (C) (non-resonant scope). Bion exception scoped. Application: CP(N-1) Stokes data. TWO-AGENT (mathematician+physicist). | Active — TWO-AGENT sev-2; awaiting Critic |
