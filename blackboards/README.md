...
| Slot | Topic | Status |
|------|-------|--------|
| 0 | V_HD on compact Lie groups with bi-invariant metrics. Prop M0.1: V_HD constant (by bi-invariance). SU(2): V_HD=-1=-4|rho|^2 (Freudenthal match). General G: value needs computation (SU(3) unverified). Rank-1 symmetric spaces: constant iff alpha(alpha-1)=0 (d=3 only among spheres). §9 intrinsic vs coordinate clarification: composition forces coord-volume trivialization; eigenvalue comparison intrinsic. | TWO-AGENT (mathematician+physicist) |
