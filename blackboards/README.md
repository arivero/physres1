...
| 5 | Rank-2 instanton lattice + mixed Stokes constants from (C). Rank-2 trivial (corollary D6.4b). NEW: C_{n,m} = C_{1,0}^n · C_{0,1}^m (leading-order, non-resonant) forced by (C). Bion exception: resonant log violates simple-pole scope. THREE-AGENT (mathematician+physicist+critic). | PROMOTED to notebooks/resurgence-composition-bridge.md §15 as RES-15; slot free. Candidate paper-edit as Lemma D6.5. |
