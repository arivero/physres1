...
| 3 | OQ1a: Composition → curvature coupling via V_HD = -R/6. Three levels (local/semi-global/global). D=4: half-density = conformal coupling. Selection principle, not gravity existence. §10: a_1=0 is UNIVERSAL (not D=4). §11: Lorentzian ADM — V_HD = spatial only, lapse in action not measure. Resolves D9.1m. | SINGLE-AGENT (physicist) |
...
| 5 | V_HD product additivity theorem: V_HD(M1 x M2) = V_HD(M1) + V_HD(M2). Constancy iff constant on each factor. S^2 x S^2 (d=4): non-constant V_HD despite constant R=4. S^3 x S^1 (d=4): unique nontrivial sphere product with constant V_HD=-1=-R/6. Catalogue of constant-V_HD products. Lorentzian connection: spatial V_HD determines ordering in static spacetimes. New: a_1(Delta_{1/2})=0 universally (mathematician §10.3). | THREE-AGENT (computationalist+physicist+mathematician) |
