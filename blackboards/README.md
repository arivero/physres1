...
| 1 | Rank-2 instanton lattice + Stokes constants from (C). Rank-2 is corollary of D6.4b (trivial). Stokes constants: (C) forces F(t,λ)=e^{c(λ)t}, S_n(t) poly deg≤n; values of c(λ) not forced. Sev-3. | Active — mathematician pass done; needs physicist §5 check |
...
| 5 | Rank-2 instanton lattice + mixed Stokes constants from (C). Rank-2 trivial (corollary D6.4b). NEW: C_{n,m} = C_{1,0}^n · C_{0,1}^m (leading-order, non-resonant) forced by (C). Bion exception: resonant log violates simple-pole scope. THREE-AGENT (mathematician+physicist+critic). | Complete — THREE-AGENT sev-2; promote to resurgence notebook §14 |
