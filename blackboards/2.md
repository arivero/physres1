...
## Context
Example 3.5b in delta-objects notes that √D → ∞ at ωΔt = π (caustic).
What does the half-density formalism say about this?

## Key Insight
The divergence of √D at caustics is a **projection artifact**, not an
intrinsic singularity. The semiclassical state lives on a Lagrangian
submanifold L ⊂ T*M, where it is a smooth half-density. The caustic
occurs when the projection π: L → M develops a fold — the pushforward
of a smooth density on L becomes singular on M.

## The Maslov Index
At each caustic crossing, the Van Vleck determinant D changes sign.
The square root √D is not single-valued along paths through caustics.
...
each caustic. The Maslov index μ counts the number of conjugate points
(with multiplicity) — equivalently, the winding number in Mp(2n).

The corrected propagator:
  K_sc ∝ |D|^{1/2} exp(iS_cl/ℏ − iπμ/2)

Each caustic contributes a phase shift of −π/2.

## Connection to Bates–Weinstein
Bates–Weinstein (1997, §4.2) shows that a solution of the homogeneous
...
the H¹(L_Grassmannian, ℤ) class of the metalinear bundle.

## Draft Remark 3.5c

> `Remark 3.5c (Caustics and the Maslov index).`
> At caustics (conjugate points where D → ∞), the semiclassical
> approximation appears to break down: the amplitude |D|^{1/2}
> diverges and √D changes sign. In the half-density framework the
> singularity is a projection artifact: the semiclassical state is
> a smooth half-density on the Lagrangian submanifold L ⊂ T*M, and
> the caustic occurs because the projection π: L → M develops a
> fold [BatesWeinstein1997]. The sign ambiguity of √D is resolved by
> the metaplectic structure: half-densities transform under the
> double cover Mp(2n) of Sp(2n), and the Maslov index μ — counting
> conjugate points along the classical path — records the accumulated
> phase correction exp(−iπμ/2). This makes the "amplitudes are
...
- The remark adds genuine value: it resolves the tension flagged by
  Example 3.5b ("√D → ∞ means breakdown") and shows it's a feature
