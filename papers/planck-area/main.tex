...
least two distinct readings: 1. \textbf{Integrality (Diophantine)
reading:} the constant is a monomial in the available couplings with
integer exponents (possibly allowing negative powers). 2.
\textbf{Perturbative analyticity reading (stronger):} the constant
admits a Taylor expansion around zero couplings, so only nonnegative
integer powers appear.
...
\texttt{Derivation\ D1.6\ (Diophantine\ form\ of\ “no\ fractional\ powers”).}
...
\([C]=\text{length}^{d/2}\) is therefore the Diophantine condition
