...
\]
with \(\alpha = -im(t_1+t_2)/(2\hbar t_1 t_2)\). The final result is
\[
...
\]
After the Gaussian evaluation, the geometric factors cancel, yielding
\[
K_0(x,z;t_1+t_2)
= \left(\frac{m}{2\pi i\hbar(t_1+t_2)}\right)^{d/2}
\exp\!\left(\frac{im|x-z|^2}{2\hbar(t_1+t_2)}\right).
...
\[
[K] = \left[\frac{m}{t}\right]^{d/2}
= [M]^{d/2}[T]^{-d/2}
= [L]^{-d},
\]
using \([m] = M\) and the dispersion relation \([E] = [M][L]^2[T]^{-2}\), which gives \([M][L]^2 = [M][L]^2\) in natural units.

Thus the prefactor \((m/t)^{d/2}\) is forced by dimensional consistency.
...
## 4.2 DeWitt-Morette Composition Law

The key structural result is that the Van Vleck determinant **composes multiplicatively** (DeWitt, 1957; Morette, 1951):
\[
\Delta(x,z) = \Delta(x,y)\,\Delta(y,z),
\]
when \(x\), \(y\), \(z\) lie on a single geodesic with the intermediate point \(y\) at the appropriate time \(t_1\).

`Derivation PN-D4.1 (Composition of the full curved-space kernel).`
The composition integral is
\[
...
\]
where \(d\mu(y) = \sqrt{|g(y)|}\,d^d y\) is the Riemannian volume element.

In the semiclassical limit (stationary-phase approximation), the dominant contribution comes from the classical path \(y = y_*(x,z;t_1)\) satisfying the geodesic equation. At this stationary point:
\[
S_{\mathrm{cl}}(x,y_*;t_1) + S_{\mathrm{cl}}(y_*,z;t_2)
= S_{\mathrm{cl}}(x,z;t_1+t_2),
\]
and the DeWitt-Morette law gives
\[
...
The \(d/2\) exponent governing the path-integral normalization also determines the **renormalization threshold** for point interactions in quantum mechanics: the dimension \(d\) at which contact interactions become non-renormalizable without regularization. This connection shows that the \(d/2\) exponent is not merely a kinematic normalization—it encodes deep information about the ultraviolet structure of the theory.
...
2. **d = 2:** The integral \(\int_0^\infty t^{-1}\,e^{iEt/\hbar}\,dt\) has a **logarithmic divergence** at \(t \to 0\):
   ...
   \]
   **Marginal renormalization**: The coupling constant runs logarithmically with the cutoff. This is the canonical example discussed in the companion satellite [RGFundamental], where the 2D delta-function potential requires a renormalized coupling \(g_R(\mu)\) with beta function
   \[
...
3. **d > 2 (especially d=3):** The integral \(\int_0^\infty t^{-d/2}\,e^{iEt/\hbar}\,dt\) has a **power-law divergence** at \(t \to 0\):
   ...
   \]
   For \(d=3\), this is \(\sim \epsilon^{-1/2}\). **Non-renormalizable** without additional structure: in \(d=3\), the delta-function potential is replaced by a **scattering length** \(a\) as the physical parameter, and the theory is regularized via self-adjoint extensions of the Laplacian (boundary conditions at \(r=0\)).
...
- For \(d > 2\), the divergence becomes power-law, requiring non-perturbative input (e.g., scattering length).
...
- **Dimension d > 2:** Point interactions require non-perturbative data (RG flow to strong coupling).
...
4. **Renormalization thresholds (Section 6):** The integral \(\int_0^\infty dt\, t^{-d/2}e^{iEt/\hbar}\) converges at \(t \to 0\) if and only if \(d < 2\), establishing the **dimensional hierarchy** of point interactions: \(d=1\) (no renormalization), \(d=2\) (marginal/logarithmic), \(d \ge 3\) (non-renormalizable without additional data). The \(d/2\) exponent is the **threshold exponent** that determines when RG flow becomes necessary.
...
3. **Half-density transformation (Section 1):** Under coordinate changes \(x \to x'(x)\), the measure transforms as \(d^d x \to |\det(\partial x'/\partial x)|\,d^d x\). For the composition integral to be coordinate-invariant, the kernel must transform as a half-density in each variable: \(K \to |\det(\partial x'/\partial x)|^{1/2}|\det(\partial y'/\partial y)|^{1/2}K\). This forces the \(d/2\) exponent when written in flat coordinates.
