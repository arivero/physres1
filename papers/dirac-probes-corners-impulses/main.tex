...
potentials} (which require renormalization and are a distinct
mathematical object).
\end{abstract}
...
\(J_k=m(v_k-v_{k-1})\). The \textit{classical} stationary condition
...
\] The prefactor \((m/(2\pi i\hbar\,\Delta t))^{N/2}\) is precisely the
half-density normalization required for the composition law to hold at
each intermediate integration --- a point treated systematically in the
...
\tightlist
\item
  \sout{Extend the single-impulse model to a sequence of \(N\) impulses
  and take the continuum limit.} Addressed in Section 4.5. The
  stochastic-forcing interpretation (random impulses with prescribed
