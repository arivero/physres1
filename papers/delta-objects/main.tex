...
\hypertarget{delta-on-the-stationary-set-deltanabla-f-and-determinant-weights}{%
\section{\texorpdfstring{3. Delta on the stationary set:
\(\delta(\nabla f)\) and determinant
weights}{3. Delta on the stationary set: \textbackslash delta(\textbackslash nabla f) and determinant weights}}\label{delta-on-the-stationary-set-deltanabla-f-and-determinant-weights}}

\hypertarget{one-dimensional-identity-deltaf}{%
\subsection{\texorpdfstring{3.1 One-dimensional identity
(\(\delta(f')\))}{3.1 One-dimensional identity (\textbackslash delta(f'))}}\label{one-dimensional-identity-deltaf}}
...
\hypertarget{a-deltaf-versus-delta-delta-of-a-derivative-vs-derivative-of-delta}{%
\subsection{\texorpdfstring{3.1a \(\delta(f')\) versus \(\delta'\):
delta of a derivative vs derivative of
delta}{3.1a \textbackslash delta(f') versus \textbackslash delta': delta of a derivative vs derivative of delta}}\label{a-deltaf-versus-delta-delta-of-a-derivative-vs-derivative-of-delta}}
...
\hypertarget{b-delta-from-point-splitting-difference-quotient-of-shifted-deltas}{%
\subsection{\texorpdfstring{3.1b \(\delta'\) from point splitting
(difference quotient of shifted
deltas)}{3.1b \textbackslash delta' from point splitting (difference quotient of shifted deltas)}}\label{b-delta-from-point-splitting-difference-quotient-of-shifted-deltas}}
...
\hypertarget{multi-dimensional-identity-deltanabla-f}{%
\subsection{\texorpdfstring{3.2 Multi-dimensional identity
(\(\delta(\nabla f)\))}{3.2 Multi-dimensional identity (\textbackslash delta(\textbackslash nabla f))}}\label{multi-dimensional-identity-deltanabla-f}}
...
\hypertarget{extremals-in-weak-form-where-delta-and-delta-appear-in-eulerlagrange}{%
\subsection{\texorpdfstring{3.4 Extremals in weak form: where \(\delta\)
and \(\delta'\) appear in
Euler--Lagrange}{3.4 Extremals in weak form: where \textbackslash delta and \textbackslash delta' appear in Euler--Lagrange}}\label{extremals-in-weak-form-where-delta-and-delta-appear-in-eulerlagrange}}
...
\texttt{Example\ 4.1\ (1D\ delta\ potential:\ resolvent\ as\ a\ rank-one\ perturbation).}
In one dimension the rank-one structure is particularly explicit. For
\(H=-\frac{\hbar^2}{2m}\frac{d^2}{dx^2}+g\,\delta(x)\) with \(g<0\)
(attractive), the free resolvent at energy \(E=-\hbar^2\kappa^2/(2m)\)
is \(G_0(x,x';E)=-\frac{m}{\hbar^2\kappa}\,e^{-\kappa|x-x'|}\). The
rank-one perturbation formula gives \[
G(x,x';E)=G_0(x,x';E)+\frac{g\,G_0(x,0;E)\,G_0(0,x';E)}{1-g\,G_0(0,0;E)}.
\] The correction term factors as \(f(x)\cdot f(x')\) with
\(f(x)=G_0(x,0;E)\) --- this is the rank-one kernel in action: the point
interaction contributes a term proportional to \(|0\rangle\langle0|\) in
the resolvent. The denominator vanishes at \(\kappa=|g|m/\hbar^2\),
yielding the unique bound state \(E=-mg^2/(2\hbar^2)\), and the residue
at this pole factors as \(\psi_b(x)\,\psi_b(x')\) with
\(\psi_b(x)=\sqrt{\kappa}\,e^{-\kappa|x|}\) --- a rank-one projector
\(|\psi_b\rangle\langle\psi_b|\)
{[}AlbeverioGesztesyHoeghKrohnHolden2005{]}. In the half-density kernel
language, the factored piece reads
\((\sqrt{\kappa}\,e^{-\kappa|x|}|dx|^{1/2})\otimes(\sqrt{\kappa}\,e^{-\kappa|x'|}|dx'|^{1/2})\),
manifestly a product of half-densities.
