...
Beyond the \((\phi,L_z)\) sector and the harmonic oscillator above,
integrable central problems admit a fuller action--angle description
(with a radial action and additional angle variables on the invariant
torus). EBK/WKB quantization makes the same structural point: the more
sharply the actions are specified, the less information remains in the
conjugate phases. Hardening that broader story into a standalone
foundations claim would require a separate study cycle to avoid
conflating (i) action--angle existence/global issues with (ii)
