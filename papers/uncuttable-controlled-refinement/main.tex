...
\] where \(B_j\) are Bernoulli numbers. Each correction involves a power
of the mesh (the ``regulator''), multiplied by endpoint derivatives
(boundary data). The leading correction \(j=1\) gives the trapezoidal
rule; including \(j=2\) gives Simpson's rule. This is the interval
counterpart of Remark 2.2's single-point tower: there the subtractions
were local (Taylor coefficients at \(x\)); here they are
boundary-localized (endpoint derivatives of \(f\)).
