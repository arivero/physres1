...
terms through order \(k-1\) and dividing by \(\varepsilon^k\) gives
\(f^{(k)}(x)/k!\) plus vanishing corrections. Each derivative is a
``counterterm-subtracted refinement limit at the next order'' --- the
analog of loop-by-loop renormalization in QFT.
